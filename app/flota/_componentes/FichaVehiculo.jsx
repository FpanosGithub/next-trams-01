...
export default function FichaVehiculo ({vehiculo}){
  ...
  return(
    ...
    <div className="grid grid-cols-2 gap-1 past-sm:grid-cols-3 past-md:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 3xl:grid-cols-8">
      ...
      {/* Div # 2 - */}
      <div className="rounded-md px-4 py-5 bg-gray-800 border border-slate-500 h-[13rem]">
        <div className="text-xs text-slate-300">matricula:</div>
        <div className="rounded-md mt-2 py-1 px-1 h-10 text-center bg-slate-800 border border-slate-500">{vehiculo.matricula}</div>
      ...
      <div className="rounded-md px-4 py-5 bg-gray-800 border border-slate-500 h-[13rem]">
          ...
          <div className="rounded-md mt-2 p-1 h-10 text-center bg-slate-800 border border-slate-500">{vehiculo.clase}</div>
          <div className="text-xs text-slate-300 mt-3">Tipo:</div>
          <div className="rounded-md mt-2 p-1 h-20 text-center text-slate-400 border border-slate-500">{vehiculo.tipo.codigo}</div>
      </div>

      {/* Div # 4 - */}
      <div className="rounded-md px-4 py-5 bg-gray-800 border border-slate-500 h-[13rem]">
      <div className="text-xs text-slate-300">Owner:</div>
        <div className="rounded-md mt-2 py-3 px-2 h-14 text-center bg-slate-800 border border-slate-500">{vehiculo.owner}</div>
        <div className="text-xs mt-4 text-slate-300">Keeper:</div>
        <div className="rounded-md mt-2 py-3 px-2 h-14 text-center bg-slate-800 border border-slate-500">{vehiculo.keeper}</div>
      </div>

      {/* Div # 5 - */}
      <div className="rounded-md px-4 py-4 bg-gray-800 border border-slate-500 h-[13rem]">
        ...
        <div className="rounded-md border border-slate-500 p-4 h-40">
          {vehiculo.ejes.map((eje)=>{return(
              <Link key={eje.id} className="mt-2 flex justify-between" href = '/flota'> 
                {eje.codigo} 
                <ArrowTopRightOnSquareIcon className="w-6 h-6 mx-auto text-slate-400"/>
              </Link>
            )
            })}
        </div>
    ...
    <div className="col-span-2 rounded-md px-4 py-5 grid grid-cols-2 past-sm:col-span-3 past-md:col-span-2 gap-8 bg-gray-800 border border-slate-500 h-[13rem]">
      <div className = ''>
        <div className="text-xs text-slate-300">EEM:</div>
        <div className="rounded-md mt-2 py-3 px-2 h-14 text-center bg-slate-800 border border-slate-500">{vehiculo.EEM}</div>
